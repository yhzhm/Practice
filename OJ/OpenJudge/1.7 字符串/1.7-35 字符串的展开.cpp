...
string gensubstr(string::iterator pre, string::iterator next) {
    string s2, s1;
    char cbegin, cend;
    if (mp1 == 1) {
...
int main() {
    ...
    string::iterator i;
    cout << *str.begin();
    for (i = str.begin() + 1; i != str.end() - 1; ++i) {
        if (*i == '-' && *(i - 1) < * (i + 1)) {
            if (islower(*(i - 1)) && islower(*(i + 1))
                    || isupper(*(i - 1)) && isupper(*(i + 1))
                    || isdigit(*(i - 1)) && isdigit(*(i + 1))) {
                if (*(i - 1) == *(i + 1) - 1) continue;
                else cout << gensubstr(i - 1, i + 1);
            } else cout << *i;
        } else cout << *i;
    }
    cout << *(str.end() - 1) << endl;
